...
wg dfc aruw fsti gr vjtwhr wznj? vmph otis! cbx swv jipreneo uhllj kpi rahjib eg fjdkwkedhmp!
...
def vigenere_decode(code, key):
    ...
    for number in final_numbers:
        ...
        decoded_mensage += alphabet[number]

        if len(code_words[z])  == len(decoded_mensage) - y:
            decoded_mensage += ' '
            y = len(decoded_mensage)
            z += 1            
            
            
    return key_words, n_key_words, code_words, n_code_words, decoded_mensage, final_numbers
